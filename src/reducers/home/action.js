...
export function actionFetchNewBlogs(genreId) {
  return function (dispatch) {
  ...
  return function (dispatch) {
    ...
    apiClient
      ...
      .then((res) => {
        var response = res.data;
        if(response.result){
          var blogsites = response.data;

          blogsites.sort(function(a, b){
            return new Date(b.blogs[0].blog_date) - new Date(a.blogs[0].blog_date);
          });

          dispatch(fetchBlogsiteSuccess(blogsites, response.total, genreId))
        } else {
