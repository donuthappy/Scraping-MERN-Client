...
const Main = (props) => {
   ...
   useEffect(() => {
      ...
      dispatch(actionFetchNewBlogs(child))
   ...
   const pageBtn = (total, genre_id) => {
      ...
      for (var i = 0; i < btnNum; i++) {
         if(i === btnNum - 1){
            if(i*50 + 1 === total){
               btnItems.push(<Button style={{'width': '100px'}} key={i} value={i} onClick={(e) => onChangePage(e, genre_id)} variant="primary">{total}</Button>);
            } else {
               btnItems.push(<Button style={{'width': '100px'}} key={i} value={i} onClick={(e) => onChangePage(e, genre_id)} variant="primary">{i*50 + 1} ~ {total}</Button>);
            }
               btnItems.push(' ')
         } else {
            btnItems.push(<Button style={{'width': '100px'}} key={i} value={i} onClick={(e) => onChangePage(e, genre_id)} variant="primary">{i*50 + 1} ~ {50}</Button>);
            btnItems.push(' ')
         }
      }
      return btnItems
                                 ...
                                 <tbody>
                                    {
                                       newBlogs.map((blog, index) => (
                                          <tr key={index}>
                                             <td width={'15%'} className="text-center mobile-response">{blog.blog_date.slice(5, 7)}月{blog.blog_date.slice(8, 10)}日&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{blog.blog_date.slice(11, 16)}</td>
                                             <td width={'65%'}><img src={blog.blogsite_id.icon} style={{ 'marginRight': '10px', 'width': '25px' }} /><a target="_blank" href={blog.link}>{blog.title}</a></td>
                                             <td width={'20%'} className="mobile-response"><a target="_blank" href={blog.blogsite_id.URL}>{blog.blogsite_id.title}</a></td>
                                          </tr>
                                       ))
                                    }
                                 </tbody>
                                 ...
                                 <Card.Header>
                                    <div className='row'>
                                       ...
                                       <div className='col-xl-10 col-md-10 col-sm-10 col-10' style={{ 'display': 'flex', 'margin': 'auto' }}>
                                          <img className='mobile-response' src={blogsite.icon} style={{ 'marginRight': '10px', 'width': '25px', 'height': '25px' }} />
                                          <span className='text-nowrap-overflow' style={{ 'fontSize': '20px', 'color': '#444' }}><a target="_blank" href={blogsite.URL}>{blogsite.title}</a></span>
                                       </div>
                                    </div>
                                 </Card.Header>
                                 <Card.Body>
                                    <div className='row'>
                                       <div className='col-xl-4 text-center margin-response'>
                                          <a className='mobile-response' target="_blank" href={blogsite.URL}><img src={'http://133.125.63.184:3000/data/uploads/'+ blogsite.image} alt="" width={'100px'} height={'130px'} /></a>
                                          <br />
                                          <br />
                                       ...
                                       <div className='col-xl-8'>
                                          <ListGroup variant="flush">
                                             {blogsite.blogs.map((blog, index) => (
                                                <ListGroup.Item key={index} className='text-nowrap-overflow'><Icon.ChevronRight color="royalblue" style={{ 'marginRight': '10px' }} /><a target="_blank" href={blog.blog_link}>{blog.blog_title}</a></ListGroup.Item>
                                             ))}
                                          </ListGroup>
                                       </div>
                                    </div>
