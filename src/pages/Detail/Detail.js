...
const Detail = (props) => {
   ...
   const pageBtn = (total) => {
      ...
      for (var i = 0; i < btnNum; i++) {
         if(i === btnNum - 1){
            if(i*50 + 1 === total){
               btnItems.push(<Button style={{'width': '100px'}} key={i} value={i} onClick={(e) => onChangePage(e, params.id)} variant="primary">{total}</Button>);
            } else {
               btnItems.push(<Button style={{'width': '100px'}} key={i} value={i} onClick={(e) => onChangePage(e, params.id)} variant="primary">{i*50 + 1} ~ {total}</Button>);
            }
            btnItems.push(' ')
         } else {
            btnItems.push(<Button style={{'width': '100px'}} key={i} value={i} onClick={(e) => onChangePage(e, params.id)} variant="primary">{i*50 + 1} ~ {50*(i+1)}</Button>);
            btnItems.push(' ')
         }
      }
      return btnItems
                  ...
                  <div className="col-xl-12">
                     <div className="appoinment-box-2" style={{ 'marginTop': '0px' }}>
                        <div className='new-blogs-header'>
                           <span style={{ 'fontSize': '20px', 'marginLeft': '50px' }}><a target="_blank" style={{'color': 'white'}} href={blogsite_link}>{blogsite_title}</a></span>
                        </div>
                        {
                           loadingBlog
                           ?
